...
# if it is near enough to another head: concat
...
adjusted_number_of_tails_list = [9.605837092497614, 9.606399099955997, 9.606680128346882, 7.685479002619189,
                                 3.8427619851594077, 1.9213866135914897, 7.685559944857391]

point_one_cube_vol = [1250000000000.0, 2500000000000.0, 5000000000000.0, 10000000000000.0, 10000000000000.0,
                      10000000000000.0, 100000000000000.0]
one_cube_vol = [125000000000.0, 250000000000.0, 500000000000.0, 1000000000000.0, 1000000000000.0, 1000000000000.0,
                10000000000000.0]
ten_cube_vol = [12500000000.0, 25000000000.0, 50000000000.0, 100000000000.0, 100000000000.0, 100000000000.0,
                1000000000000.0]
one_hundred_cube_vol = [1250000000.0, 2500000000.0, 5000000000.0, 10000000000.0, 10000000000.0, 10000000000.0,
                        100000000000.0]
...
file = open("concat_snapshot_model_output.txt", "a")
...
def run(simulated_vols, trials, tails_list):
    ...
    for i in range(size):
        ...
        sum_cir_prob_thru_trials = 0
        sum_concat_prob_thru_trials = 0
        ...
        for j in range(1, trials + 1):
            ...
            sum_cir_prob_thru_trials += temp_cir_prob

            temp_concat_prob = concat_event / tails_count
            sum_concat_prob_thru_trials += temp_concat_prob
        ...
        final_cir_prob = sum_cir_prob_thru_trials / trials
        final_concat_prob = sum_concat_prob_thru_trials / trials

        cir_output.append(final_cir_prob)
        concat_output.append(final_concat_prob)
...
for i in range(size - 1, size):
    ...
    file.write(f"{i} cir: {temp_cir} concat: {temp_concat} \n")
