...
simulated_6_cant_go_back = [395 / 25000000, 127 / 25000000, 39 / 25000000, 14 / 25000000]
...
for i in range(0, size):
    ...
    for j in range(0, num_lens):
        ax1.annotate(f"{round(model1[i][j], 14)}", (length[j], model1[i][j]), xycoords='data', xytext=(length[j] * 1.00000001, model1[i][j] * 1.01))
...
ax1.plot(length2, simulated_6_cant_go_back, color = 'brown', marker = 'o', label = "6 dir cant go back")
for j in range(0, len(simulated_6_cant_go_back)):
    ax1.annotate(f"{round(simulated_6_cant_go_back[j], 10)}", (length2[j], simulated_6_cant_go_back[j]), xycoords='data', xytext=(length2[j] * 1.00000001, simulated_6_cant_go_back[j] * 1.2))

ax1.set_xlabel('length')
ax1.set_yscale('log', base = 10)
ax1.set_ylabel('Circularization vs Concatemerization', color = 'black')
