    ...
    for i in range(num_trails):
        for j in range(num_tails):
            ...
            if comp_sphere(tail, radius):
                concat += 1
            if i % 100000 == 0:
                print(f"total_tail = {num_tails} trial_num {i} tail_num {j} concat {concat}")
    return concat

concat = run(num_tails, side_len / 2, sphere_radius)
file = open("big_cube_concat_24_square", "w")

file.write(f"tail: {num_tails} concat: {concat} trials {num_trails}")