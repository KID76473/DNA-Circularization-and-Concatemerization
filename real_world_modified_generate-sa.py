...
try:
    ...
    infile = sys.argv[2]
    output_top_file_name = str(sys.argv[3])
    output_init_config_file_name = str(sys.argv[4])
except:
    print("Usage: %s <%s> <%s> <%s>" % (sys.argv[0], "box size", "file with sequences", "topo_file_output_name", "init_config_file_output_name"), file=sys.stderr)
    sys.exit(1)
...
def read_strands(filename):
    ...
    try:
        out = open(f"{output_top_file_name}", "w")
    except:
        print("Could not open topological file for writing. Aborting", file=sys.stderr)
        sys.exit(4)
    ...
    try:
        outfile = open(f"{output_init_config_file_name}", 'w')
    except:
        print("Could not open initial config file for writing.  Aborting", file=sys.stderr)
        sys.exit(5)
    ...
    outfile.close()
    print(f"## ALL DONE. just generated {output_init_config_file_name} and {output_top_file_name}", file=sys.stderr)
